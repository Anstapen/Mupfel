...
bool Imager::Load(const std::string& in_handle, const std::string& path, Mupfel::ResourceManager* mngr)
{
	auto img_handle = Images::Load(path, mngr).transform([&in_handle](ImageHandle handle) { image_map[in_handle] = handle; });

	return img_handle.has_value();
}

bool Imager::LoadAnimated(
	const std::string&		  in_handle,
	const std::string&		  path,
	const ImageSpecification& spec,
	Mupfel::ResourceManager*  mngr)
{
	auto img_handle =
		Images::LoadAnimated(path, spec, mngr).transform([&in_handle](ImageHandle handle) { image_map[in_handle] = handle; });

	return img_handle.has_value();
}

bool Imager::LoadSpriteSheet(
	const std::string&		  in_handle,
	const std::string&		  path,
	const ImageSpecification& spec,
	Mupfel::ResourceManager*  mngr)
{
	auto img_handle =
		Images::LoadSpriteSheet(path, spec, mngr)
